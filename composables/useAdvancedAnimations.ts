...
export function useAdvancedAnimations() {
  ...
  const parallaxScroll = (selector: string | Element, speed: number = 0.5) => {
    if (!$gsap || !$ScrollTrigger) return;
  ...
  const animateCounter = (
    ...
    const { duration = 2, delay = 0, ease = "power2.out" } = options;
  ...
  const pageTransition = (
    ...
    tl.to(outElement, outProps).to(inElement, inProps);
...
export function useCustomCursor() {
  ...
  const initCursor = () => {
    ...
    cursor.className =
      "custom-cursor fixed pointer-events-none z-50 w-5 h-5 rounded-full bg-blue-500/50 backdrop-blur-sm";
    ...
    const interactiveElements = document.querySelectorAll(
      'button, a, [role="button"], .hover-cursor',
    );
  ...
  const updateCursorState = (state: "normal" | "hover" | "click") => {
    ...
    cursorElement.value.className =
      "custom-cursor fixed pointer-events-none z-50 rounded-full backdrop-blur-sm";

    switch (state) {
      case "hover":
        cursorElement.value.className +=
          " w-10 h-10 bg-blue-500/30 border-2 border-blue-500/80";
        break;
      case "click":
