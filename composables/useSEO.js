export const useSEO = () => {
  ...
  const generateArticleSchema = (article) => {
    return {
      ...
      image: article.image ? `${config.public.siteUrl}${article.image}` : undefined,
      mainEntityOfPage: {
        '@type': 'WebPage',
  ...
  const generateOrganizationSchema = () => {
    return {
      ...
      description: 'Professional SEO audit tool with AI-powered insights for agencies and businesses.',
      sameAs: [
  ...
  const setPageMeta = ({
    ...
    const meta = [
      ...
      { property: 'og:url', content: `${config.public.siteUrl}${useRoute().path}` },
    ...
    if (image) {
      const imageUrl = image.startsWith('http') ? image : `${config.public.siteUrl}${image}`
      meta.push(
    ...
    useHead({
      ...
      meta,
      link: [
        { rel: 'canonical', href: `${config.public.siteUrl}${useRoute().path}` }
      ]
    })
  }

  // Generate robots meta tag
  const setRobotsMeta = (index = true, follow = true, noarchive = false, nosnippet = false) => {
    const directives = []
    ...
    useHead({
      meta: [
        { name: 'robots', content: directives.join(', ') }
      ]
    })
  }
