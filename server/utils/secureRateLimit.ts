...
export async function checkRateLimit(
  ...
  try {
    ...
    if (rateData.blocked && rateData.blockExpires && rateData.blockExpires > now) {
      return {
        allowed: false,
        blocked: true,
    ...
    if (rateData.count >= config.maxRequests) {
      ...
      if (rateData.violations >= 3 && config.blockDuration) {
        ...
        logger.warn('[SECURITY] User blocked due to repeated rate limit violations', {
          identifier,
...
export function setRateLimitHeaders(event: H3Event, result: RateLimitResult) {
  ...
  if (result.blocked) {
    ...
    if (result.blockExpires) {
      setHeader(event, 'X-RateLimit-Block-Expires', Math.ceil(result.blockExpires / 1000).toString())
    }
  }
}
...
function cleanupExpiredEntries(now: number) {
  for (const [key, data] of rateLimitStore.entries()) {
    if (data.resetTime < now && (!data.blockExpires || data.blockExpires < now)) {
