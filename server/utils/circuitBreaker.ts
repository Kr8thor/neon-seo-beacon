...
export class CircuitBreaker {
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    ...
    if (this.stats.state === CircuitState.OPEN) {
      if (Date.now() < this.nextAttempt) {
        console.log(`[CIRCUIT-BREAKER] ${this.name}: Circuit is OPEN, rejecting call`)
        return this.handleOpenCircuit()
      } else {
        // Try to transition to HALF_OPEN
        this.stats.state = CircuitState.HALF_OPEN
        console.log(`[CIRCUIT-BREAKER] ${this.name}: Circuit transitioning to HALF_OPEN`)
      }
    }
  ...
  private onSuccess(): void {
    ...
    if (this.stats.state === CircuitState.HALF_OPEN) {
      if (this.stats.successes >= this.config.successThreshold) {
        ...
        console.log(`[CIRCUIT-BREAKER] ${this.name}: Circuit recovered, state: CLOSED`)
      }
    }
  }
  ...
  private openCircuit(): void {
    ...
    this.stats.successes = 0
    console.log(`[CIRCUIT-BREAKER] ${this.name}: Circuit OPENED due to failures`)
  }
  ...
  private async handleOpenCircuit<T>(): Promise<T> {
    ...
    throw new Error(`Service unavailable: ${this.name} circuit breaker is OPEN`)
  }
