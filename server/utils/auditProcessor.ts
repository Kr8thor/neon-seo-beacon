...
async function processJobAsync(job: AuditJob) {
  ...
  console.log(`[AUDIT-PROCESSOR] Starting job ${job.id} for audit ${job.auditId} (attempt ${job.attempts}/${job.maxAttempts})`)
  ...
  try {
    ...
    console.log(`[AUDIT-PROCESSOR] Job ${job.id} completed successfully (score: ${score})`)
    ...
    if (job.attempts >= job.maxAttempts) {
      ...
      await addAuditProgress(job.auditId, 5, 5, `Analysis failed: ${error.message}`)
    } else {
      // Schedule retry with exponential backoff
      job.status = 'pending'
      job.nextRetry = Date.now() + (Math.pow(2, job.attempts - 1) * 5000) // 5s, 10s, 20s, 40s...
      console.log(`[AUDIT-PROCESSOR] Job ${job.id} will retry in ${(job.nextRetry - Date.now()) / 1000}s`)
    }
  } finally {
...
export function addAuditJob(auditId: string, url: string, type: string = 'full'): string {
...
async function updateAuditStatus(auditId: string, status: string, error?: string) {
  try {
    ...
    console.log(`[AUDIT-PROCESSOR] Updated audit ${auditId} status to ${status}`)
  } catch (error) {
...
async function addAuditProgress(auditId: string, step: number, totalSteps: number, message: string, data?: any) {
  try {
    const supabase = createSupabaseClient()
    ...
    console.log(`[AUDIT-PROCESSOR] Audit ${auditId} progress: ${step}/${totalSteps} - ${message}`)
  } catch (error) {
...
async function generateRecommendations(analysisResult: any) {
  ...
  } else if (analysisResult.title.length < 30 || analysisResult.title.length > 60) {
    recommendations.push({
      category: 'Title Tag',
      priority: 'medium',
      issue: 'Title length not optimal',
      recommendation: 'Optimize title length to 30-60 characters for better search visibility',
...
function calculateSEOScore(analysisResult: any): number {
  ...
  // Title tag (15 points)
  if (analysisResult.title) {
    if (analysisResult.title.length >= 30 && analysisResult.title.length <= 60) {
      score += 15
  ...
  if (analysisResult.metaDescription) {
    if (analysisResult.metaDescription.length >= 120 && analysisResult.metaDescription.length <= 160) {
      score += 15
  ...
  if (analysisResult.images) {
    const altPercentage = analysisResult.images.total > 0 ? 
      (analysisResult.images.withAlt / analysisResult.images.total) * 100 : 100
  ...
  if (technical.structuredData && technical.structuredData.count > 0) score += 5
  if (technical.openGraph && Object.keys(technical.openGraph).length > 3) score += 4
...
async function updateAuditResults(auditId: string, results: any, score: number, processingTime: number) {
  try {
    const supabase = createSupabaseClient()
    ...
    console.log(`[AUDIT-PROCESSOR] Updated audit ${auditId} with results (score: ${score})`)
  } catch (error) {
