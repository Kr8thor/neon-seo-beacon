<template>
  <div
    class="audit-results-page min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-blue-900 dark:to-indigo-900"
  >
    <!-- Header -->
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="loading" class="text-center py-16">
        <div
          class="inline-block animate-spin w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full mb-4"
        ></div>
      ...
      <div
        v-else-if="
          audit && (audit.status === 'processing' || audit.status === 'queued')
        "
        class="space-y-8"
      >
        <!-- Progress Card -->
        <div class="glass rounded-3xl p-8 text-center">
          <div
            class="w-16 h-16 mx-auto mb-6 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full flex items-center justify-center"
          >
            <svg
      ...
      <div v-else-if="audit && audit.status === 'completed'" class="space-y-8">
        ...
        <div class="glass rounded-3xl p-8">
          <div class="text-center mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-400">
              ...
              {{ formatProcessingTime(audit.processing_time_ms) }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center">
              <div class="text-2xl font-bold text-green-600">
                {{ audit.results?.technical?.passes || 0 }}
              </div>
            ...
            <div class="text-center">
              <div class="text-2xl font-bold text-yellow-600">
                {{ audit.results?.technical?.warnings || 0 }}
              </div>
              <div class="text-sm text-gray-600 dark:text-gray-400">
                Warnings
              </div>
            ...
            <div class="text-center">
              <div class="text-2xl font-bold text-red-600">
                {{ audit.results?.technical?.errors || 0 }}
              </div>
        ...
        <div v-if="audit.results" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="glass rounded-2xl p-6">
            ...
            <div class="space-y-4">
              <div class="flex items-start space-x-3">
                <div
                  class="flex-shrink-0 w-5 h-5 rounded-full mt-0.5"
                  :class="audit.results.title ? 'bg-green-500' : 'bg-red-500'"
                ></div>
                <div class="flex-1">
              ...
              <div class="flex items-start space-x-3">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span
                      v-if="audit.results.metaDescription"
                      class="ml-2 text-xs text-gray-500"
                    >
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-center">
                ...
                <span class="font-semibold">{{
                  audit.results.h1Tags?.length || 0
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300">H2 Tags</span>
                <span class="font-semibold">{{
                  audit.results.h2Tags?.length || 0
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300">Images</span>
                <span class="font-semibold">{{
                  audit.results.images?.total || 0
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300"
                  >Images with Alt Text</span
                >
                <span
                  class="font-semibold"
                  :class="getAltTextColor(audit.results.images)"
                >
                  {{ audit.results.images?.withAlt || 0 }} /
                  {{ audit.results.images?.total || 0 }}
                </span>
              </div>
            </div>
          ...
          <div class="glass rounded-2xl p-6">
            ...
            <div class="space-y-3">
              <div
                v-for="check in technicalChecks"
                :key="check.name"
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-center">
                ...
                <span
                  class="font-semibold"
                  :class="
                    getPerformanceColor(audit.results.performance?.loadTime)
                  "
                >
                  {{ audit.results.performance?.loadTime || 0 }}ms
                </span>
              ...
              <div class="flex justify-between items-center">
                ...
                <span class="font-semibold text-green-600">{{
                  audit.results.performance?.status || "N/A"
                }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-700 dark:text-gray-300"
                ...
                >
                <span class="font-semibold">{{
                  audit.results.performance?.compression || "None"
                }}</span>
              </div>
            </div>
          </div>
      ...
      <div
        v-else-if="audit && audit.status === 'error'"
      ...
      >
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-4">
          {{ audit.error || "An unknown error occurred during the audit." }}
        </p>
        <div class="flex justify-center space-x-4">
          <button
            @click="retryAudit"
...
const technicalChecks = computed(() => {
  ...
  return [
    ...
    { name: "Structured Data", passed: technical.structuredData?.count > 0 },
    {
      ...
      passed: Object.keys(technical.openGraph || {}).length > 0,
    },
  ];
});
...
async function loadAudit() {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (
        audit.value.status === "processing" ||
        audit.value.status === "queued"
      ) {
...
setPageMeta({
  ...
  description:
    "View detailed SEO audit results and recommendations for your website optimization.",
});
</script>
