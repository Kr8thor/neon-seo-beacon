<template>
  ...
  >
    <!-- Cursor Dot -->
    <div
      class="cursor-dot absolute inset-0 rounded-full transition-all duration-150"
      :class="dotClasses"
    ></div>
...
const cursorState = ref<
  "normal" | "hover" | "click" | "text" | "pointer" | "grab"
>("normal");
...
const addTrailParticle = (x: number, y: number) => {
  ...
  setTimeout(() => {
    ...
    if (index > -1 && trailParticles.value[index]) {
      ...
      setTimeout(() => {
        const finalIndex = trailParticles.value.findIndex(
          (p) => p.id === particle.id,
        );
        if (finalIndex > -1) {
...
const initCursor = () => {
  ...
  const interactiveElements = document.querySelectorAll(
    'a, button, [role="button"], .cursor-pointer, .hover-cursor',
  );
