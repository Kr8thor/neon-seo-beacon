...
describe("SEO Utils", () => {
  ...
  describe("generateSlug", () => {
    it("should generate SEO-friendly slugs", () => {
      ...
      expect(generateSlug("SEO Guide: Best Practices")).toBe(
  ...
  describe("calculateReadingTime", () => {
    it("should calculate reading time correctly", () => {
      ...
      expect(calculateReadingTime(longText + " " + longText)).toBe(
        "2 min read",
      ); // 400 words
    });
  ...
  describe("validateMetaDescription", () => {
    it("should validate optimal meta descriptions", () => {
      const goodDescription =
        "This is a well-written meta description that provides a clear summary of the page content and falls within the optimal length range.";
    ...
    it("should detect too short meta descriptions", () => {
      ...
      expect(result.feedback).toBe(
        "Meta description is too short (minimum 120 characters)",
      );
    });
    ...
    it("should detect too long meta descriptions", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.feedback).toBe(
        "Meta description is too long (maximum 160 characters)",
      );
    });
  });
  ...
  describe("validateTitle", () => {
    ...
    it("should detect too short titles", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.feedback).toBe(
        "Title is too short (minimum 30 characters)",
      );
    });
  ...
  describe("isInternalUrl", () => {
    ...
    it("should identify external URLs", () => {
      ...
      expect(
        isInternalUrl("https://subdomain.other.com", "https://example.com"),
      ).toBe(false);
    });
  ...
  describe("extractKeywords", () => {
    it("should extract keywords from text", () => {
      const text =
        "This is a sample text about search engine optimization and keyword extraction for testing purposes";
  ...
  describe("calculateContentDensity", () => {
    ...
    it("should prefer text content over markup", () => {
      ...
      const lowDensity = calculateContentDensity(
  ...
  describe("isCrawlable", () => {
    ...
    it("should respect disallow rules", () => {
      const robotsTxt = "User-agent: *\nDisallow: /admin\nDisallow: /private";
      expect(isCrawlable("https://example.com/admin/page", robotsTxt)).toBe(
        false,
      );
      expect(isCrawlable("https://example.com/public/page", robotsTxt)).toBe(
        true,
      );
    });
    ...
    it("should handle specific user agents", () => {
      ...
      expect(
        isCrawlable("https://example.com/special", robotsTxt, "googlebot"),
      ).toBe(false);
      expect(
        isCrawlable("https://example.com/special", robotsTxt, "bingbot"),
      ).toBe(true);
    });
  });
  ...
  describe("validateImageAlt", () => {
    ...
    it("should reject too short alt text", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.feedback).toBe(
        "Alt text is too short (minimum 10 characters)",
      );
    });
    ...
    it("should reject too long alt text", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.feedback).toBe(
        "Alt text is too long (maximum 125 characters)",
      );
    });
    ...
    it("should reject filename-based alt text", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.feedback).toBe(
        "Alt text appears to be just the filename - make it more descriptive",
      );
    });
  });
});
