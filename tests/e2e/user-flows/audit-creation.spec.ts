...
test.describe('Audit Creation Flow', () => {
  ...
  test('should handle invalid URL gracefully', async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("Create"), button:has-text("Audit"), [data-testid*="create"]').first()
    ...
    if (await createButton.isVisible()) {
      ...
      if (await urlInput.isVisible()) {
        ...
        const submitButton = page.locator('button[type="submit"], button:has-text("Submit"), button:has-text("Analyze")').first()
        
        if (await submitButton.isVisible()) {
          await submitButton.click()
          
          // Check for error message
          await expect(page.locator('text=/invalid/i, text=/error/i').first()).toBeVisible({ timeout: 5000 })
        }
      }
    }
  })

  test('should show loading states during audit processing', async ({ page }) => {
    await page.goto('/dashboard')
    
    // This test will check for loading indicators when they're implemented
    const createButton = page.locator('button:has-text("Create"), button:has-text("Audit")').first()
    ...
    if (await createButton.isVisible()) {
      ...
      if (await urlInput.isVisible()) {
        ...
        if (await submitButton.isVisible()) {
          ...
          const loadingIndicator = page.locator('[data-testid*="loading"], .loading, text=/processing/i, text=/analyzing/i').first()
