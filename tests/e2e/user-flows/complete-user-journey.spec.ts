...
test.describe('Complete User Journey Testing', () => {
  ...
  test('complete user registration to audit completion flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="progress-step"]')).toContainText('Fetching')
    await page.waitForFunction(() => {
      const progressBar = document.querySelector('[data-testid="progress-bar"]')
      return progressBar && parseInt(progressBar.getAttribute('value') || '0') > 20
    }, { timeout: 10000 })
    
    // Step 7: Wait for completion and verify results
    await page.waitForSelector('[data-testid="audit-completed"]', { timeout: 60000 })
  ...
  test('authenticated user workflow - login and multiple audits', async ({ page }) => {
    ...
    const urls = ['https://google.com', 'https://github.com', 'https://nuxt.com']
    ...
    await page.click('[data-testid="audit-history-tab"]')
    await expect(page.locator('[data-testid="audit-item"]')).toHaveCount.toBeGreaterThan(1)
  })
})
