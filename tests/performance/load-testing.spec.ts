...
test.describe("Advanced Performance Testing", () => {
  test("concurrent user load test", async ({ page, context }) => {
    ...
    const contexts = await Promise.all(
      Array(concurrentUsers)
        .fill(null)
        .map(() => page.context().browser()!.newContext()),
    );
    ...
    const auditPromises = pages.map(async (userPage, index) => {
      ...
      await userPage.waitForSelector('[data-testid="processing-status"]', {
        timeout: 10000,
      });
    ...
    for (const userPage of pages) {
      await expect(
        userPage.locator('[data-testid="processing-status"]'),
      ).toBeVisible();
    }
  ...
  test("memory leak detection", async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await page
        .click('[data-testid="cancel-audit"]', { timeout: 5000 })
        .catch(() => {});
